"use strict";exports.id=373,exports.ids=[373],exports.modules={7779:(e,t,r)=>{r.d(t,{HE:()=>i,HY:()=>l,T4:()=>o,T9:()=>s,vs:()=>d});let a=[{value:"newest",label:"Terbaru"},{value:"price_low",label:"Harga Terendah ke Tertinggi"},{value:"price_high",label:"Harga Tertinggi ke Terendah"}];class i{static buildUrl(e,t){return`${e}${t}`}static getDefaultRequestConfig(){return{method:"POST",headers:{"Content-Type":"application/json"}}}static async loadCategories(e){try{let t=this.buildUrl(e.baseUrl,e.endpoints.categories),r={...this.getDefaultRequestConfig(),...e.defaultRequestConfig,body:JSON.stringify({})},a=await fetch(t,r);if(!a.ok)throw Error(`HTTP ${a.status}: ${a.statusText}`);let i=await a.json();if(!i.success)throw Error(i.error||"Categories API returned success: false");let n=[];if(i.data&&Array.isArray(i.data.categories))n=i.data.categories;else if(i.data&&Array.isArray(i.data))n=i.data;else throw Error("Invalid categories response format");return n.filter(e=>{let t=e.level||0;return 0===t})}catch(e){return[]}}static async getCategoryBySlug(e,t){try{let r=this.buildUrl(t.baseUrl,t.endpoints.categories),a={...this.getDefaultRequestConfig(),...t.defaultRequestConfig,body:JSON.stringify({category_slug:e})},i=await fetch(r,a);if(!i.ok)throw Error(`HTTP ${i.status}: ${i.statusText}`);let n=await i.json();if(!n.success)throw Error(n.error||"Category API returned success: false");if(n.data&&n.data.category)return n.data.category;return null}catch(e){return null}}}class n{static{this.callbackRegistry=new Map}static buildSearchInput(e="",t="Cari produk...",r){let a=`search-input-${Date.now()}-${Math.random().toString(36).substr(2,9)}`;if(r){let e=`handleSearch_${a}`;this.callbackRegistry.set(e,r),window[e]=t=>{let r=this.callbackRegistry.get(e);r&&r(t)}}return`
    ...
    `}static buildSortSelector(e="newest",t){let r=`sort-select-${Date.now()}-${Math.random().toString(36).substr(2,9)}`;if(t){let e=`handleSort_${r}`;this.callbackRegistry.set(e,t),window[e]=t=>{let r=this.callbackRegistry.get(e);r&&r(t)}}let i=a.map(t=>{let r=e===t.value?"selected":"";return`<option value="${t.value}" ${r}>${t.label}</option>`}).join("");return`
      <div class="flex justify-center">
        <div class="flex items-center gap-4 bg-white rounded-lg shadow-sm border p-3">
    ...
    `}static buildCompleteFilterSection(e,t,r,a={}){let{showSearch:i=!0,showPriceRange:n=!0,showCategoryFilter:o=!0,showSort:l=!0,categoryLinkMode:s=!1}=a,d=[];return i&&d.push(this.buildSearchInput(t.searchQuery,"Cari produk...",r.onSearchChange)),n&&d.push(this.buildPriceRangeFilter(t.priceRange,r.onPriceRangeChange)),o&&d.push(this.buildCategoryFilter(e,t.categoryId,r.onCategoryChange,s)),l&&d.push(this.buildSortSelector(t.sortOrder,r.onSortChange)),`
      <div class="mb-8 space-y-6">
        ${d.join("")}
      </div>
    `}static cleanup(){this.callbackRegistry.forEach((e,t)=>{window[t]&&delete window[t]}),this.callbackRegistry.clear()}}let o=e=>{let t="string"==typeof e?parseFloat(e):e;return new Intl.NumberFormat("id-ID").format(t)},l=e=>{let t=parseFloat(e.price_regular)||0,r=parseFloat(e.price_discount)||0;return r>0&&r<t?r:t},s=e=>{let t=parseFloat(e.price_regular)||0,r=parseFloat(e.price_discount)||0;return r>0&&r<t},d=e=>{let t=parseFloat(e.price_regular)||0,r=parseFloat(e.price_discount)||0;return s(e)?Math.round((t-r)/t*100):0}},7373:(e,t,r)=>{r.d(t,{initializeProductsSection:()=>m});var a=r(7779),i=r(4362);let n={sectionId:"products-section",loadingId:"products-loading",gridId:"products-grid",filtersId:"products-filters",searchId:"products-search",containerClass:"container mx-auto px-4",title:"Daftar Produk",subtitle:"Temukan berbagai produk berkualitas untuk kebutuhan Anda"};class o{constructor(){this.baseUrl="/products",this.currentParams=new URLSearchParams}getCurrentParams(){let e=new URLSearchParams(window.location.search);return{page:parseInt(e.get("page"))||1,search:e.get("search")||"",category:e.get("category")||"all",min_price:e.get("min_price")?parseFloat(e.get("min_price")):null,max_price:e.get("max_price")?parseFloat(e.get("max_price")):null,sort:e.get("sort")||"recommended"}}updateURL(e){let t=new URLSearchParams;e.page&&e.page>1&&t.set("page",e.page.toString()),e.search&&e.search.trim()&&t.set("search",e.search.trim()),e.category&&"all"!==e.category&&t.set("category",e.category),null!==e.min_price&&void 0!==e.min_price&&t.set("min_price",e.min_price.toString()),null!==e.max_price&&void 0!==e.max_price&&t.set("max_price",e.max_price.toString()),e.sort&&"recommended"!==e.sort&&t.set("sort",e.sort);let r=t.toString()?`${this.baseUrl}?${t.toString()}`:this.baseUrl;r!==window.location.pathname+window.location.search&&window.history.pushState({path:r},"",r)}initializeFromURL(){return this.getCurrentParams()}}class l{constructor(){this.scrollOffset=170,this.scrollDuration=400}scrollToTop(e=!0){e?window.scrollTo({top:0,behavior:"smooth"}):window.scrollTo(0,0)}scrollToProducts(e=!0){let t=document.getElementById(n.sectionId);if(t){let r=t.getBoundingClientRect(),a=window.pageYOffset+r.top-this.scrollOffset;e?window.scrollTo({top:a,behavior:"smooth"}):window.scrollTo(0,a)}else this.scrollToTop(e)}setupAutoScrollOnLoad(){"loading"===document.readyState?document.addEventListener("DOMContentLoaded",()=>{this.scrollToTop(!1)}):this.scrollToTop(!1),window.addEventListener("popstate",e=>{this.scrollToTop(!0),this.handleURLChange()})}async handleURLChange(){let e=s.getCurrentParams();p.setPage(e.page),p.setSearchQuery(e.search),p.setCategoryFilter(e.category),p.setPriceRange(e.min_price,e.max_price),p.setSortBy(e.sort),this.syncUIWithState(),E(),g.updateBadge(),await _(!1)}syncUIWithState(){let e={search:document.getElementById("search-input"),minPrice:document.getElementById("min-price"),maxPrice:document.getElementById("max-price"),sort:document.getElementById("sort-select")};e.search&&(e.search.value=p.searchQuery),e.minPrice&&(e.minPrice.value=p.priceRange.min||""),e.maxPrice&&(e.maxPrice.value=p.priceRange.max||""),e.sort&&(e.sort.value=p.sortBy);let t={search:document.getElementById("search-input-mobile"),minPrice:document.getElementById("min-price-mobile"),maxPrice:document.getElementById("max-price-mobile"),sort:document.getElementById("sort-select-mobile")};t.search&&(t.search.value=p.searchQuery),t.minPrice&&(t.minPrice.value=p.priceRange.min||""),t.maxPrice&&(t.maxPrice.value=p.priceRange.max||""),t.sort&&(t.sort.value=p.sortBy),document.querySelectorAll(".category-filter-item").forEach(e=>{e.classList.remove("active"),e.dataset.category===p.currentCategoryFilter&&e.classList.add("active")})}}let s=new o,d=new l;class c{constructor(){this.categories=[],this.currentProducts=[],this.allProducts=[],this.isLoading=!1,this.totalItems=0,this.page=1,this.limit=20,this.currentCategoryFilter="all",this.searchQuery="",this.priceRange={min:null,max:null},this.sortBy="recommended"}setPage(e){this.page=e,this.updateURL()}setCategoryFilter(e){this.currentCategoryFilter=e,this.page=1,this.updateURL()}setSearchQuery(e){this.searchQuery=e,this.page=1,this.updateURL()}setPriceRange(e,t){this.priceRange={min:e,max:t},this.page=1,this.updateURL()}setSortBy(e){this.sortBy=e,this.page=1,this.updateURL()}reset(){this.currentProducts=[],this.allProducts=[],this.totalItems=0,this.page=1,this.currentCategoryFilter="all",this.searchQuery="",this.priceRange={min:null,max:null},this.sortBy="recommended",this.updateURL()}updateURL(){s.updateURL({page:this.page,search:this.searchQuery,category:this.currentCategoryFilter,min_price:this.priceRange.min,max_price:this.priceRange.max,sort:this.sortBy})}initializeFromURL(){let e=s.initializeFromURL();this.page=e.page,this.searchQuery=e.search,this.currentCategoryFilter=e.category,this.priceRange.min=e.min_price,this.priceRange.max=e.max_price,this.sortBy=e.sort}getFilterState(){return{categoryId:"all"===this.currentCategoryFilter?null:this.currentCategoryFilter,searchQuery:this.searchQuery,priceRange:this.priceRange,sortBy:this.sortBy,page:this.page,limit:this.limit}}}let p=new c;class u{constructor(){this.isOpen=!1,this.overlay=null,this.panel=null,this.filterBtn=null,this.badge=null}init(){this.overlay=document.getElementById("filter-overlay"),this.panel=document.getElementById("filter-panel"),this.filterBtn=document.getElementById("mobile-filter-btn"),this.badge=document.getElementById("filter-badge"),this.setupEventListeners(),this.updateBadge()}setupEventListeners(){this.filterBtn&&this.filterBtn.addEventListener("click",()=>this.open());let e=document.getElementById("filter-panel-close");e&&e.addEventListener("click",()=>this.close()),this.overlay&&this.overlay.addEventListener("click",e=>{e.target===this.overlay&&this.close()});let t=document.getElementById("filter-apply-mobile");t&&t.addEventListener("click",()=>this.applyFilters());let r=document.getElementById("filter-reset-mobile");r&&r.addEventListener("click",()=>this.resetFilters()),document.addEventListener("keydown",e=>{"Escape"===e.key&&this.isOpen&&this.close()})}open(){if(this.overlay&&this.panel){this.isOpen=!0,document.body.classList.add("filter-panel-open"),this.overlay.classList.add("active"),this.panel.classList.add("active");let e=document.getElementById("search-input-mobile");e&&setTimeout(()=>e.focus(),300)}}close(){this.overlay&&this.panel&&(this.isOpen=!1,document.body.classList.remove("filter-panel-open"),this.overlay.classList.remove("active"),this.panel.classList.remove("active"))}async applyFilters(){let e=document.getElementById("search-input-mobile")?.value.trim()||"",t=document.getElementById("min-price-mobile")?.value?parseFloat(document.getElementById("min-price-mobile").value):null,r=document.getElementById("max-price-mobile")?.value?parseFloat(document.getElementById("max-price-mobile").value):null,a=document.getElementById("sort-select-mobile")?.value||"recommended";if(null!==t&&null!==r&&t>r){alert("Harga minimum tidak boleh lebih besar dari harga maksimum");return}p.setSearchQuery(e),p.setPriceRange(t,r),p.setSortBy(a),this.syncWithDesktopInputs(e,t,r,a),E(),this.updateBadge(),this.close(),await _()}async resetFilters(){["search-input-mobile","min-price-mobile","max-price-mobile"].forEach(e=>{let t=document.getElementById(e);t&&(t.value="")});let e=document.getElementById("sort-select-mobile");e&&(e.value="recommended"),p.reset(),this.syncWithDesktopInputs("",null,null,"recommended"),E(),this.updateBadge(),await _()}syncWithDesktopInputs(e,t,r,a){let i={search:document.getElementById("search-input"),minPrice:document.getElementById("min-price"),maxPrice:document.getElementById("max-price"),sort:document.getElementById("sort-select")};i.search&&(i.search.value=e),i.minPrice&&(i.minPrice.value=t||""),i.maxPrice&&(i.maxPrice.value=r||""),i.sort&&(i.sort.value=a)}updateBadge(){if(!this.badge)return;let e=0;p.searchQuery&&e++,(null!==p.priceRange.min||null!==p.priceRange.max)&&e++,"recommended"!==p.sortBy&&e++,e>0?(this.badge.textContent=e,this.badge.classList.remove("hidden")):this.badge.classList.add("hidden")}}let g=new u,m=async(e="products-section")=>{let t=document.getElementById(e);t&&(d.setupAutoScrollOnLoad(),t.innerHTML=h(),p.initializeFromURL(),await k())},h=()=>`
  ...
  `,b=()=>`
    <div class="text-center mb-8">
      <h2 class="text-3xl font-bold text-gray-800 mb-4">
        ${n.title.split(" ").map((e,t)=>1===t?`<span class="text-green-700">${e}</span>`:e).join(" ")}
      </h2>
  ...
  `,f=()=>`
    ...
    <div class="search-filters-section desktop-filters">
      <div class="boxed-container">
        <div id="${n.searchId}" class="search-filters-container">
  ...
  `,y=()=>`
    ...
    <div class="boxed-container mb-8">
      <div id="${n.filtersId}" class="category-filters-container">
        <!-- Header Kategori -->
  ...
  `,x=()=>{let e=Array.from({length:10},(e,t)=>`
  ...
  `).join("");return`
    <div id="${n.loadingId}" class="hidden">
      <div class="boxed-container">
  ...
  `},v=()=>`<div id="${n.gridId}" class="hidden"></div>`,w=()=>`
    <style>
  ...
  `,k=async()=>{try{F(),await B(),P(),$(),d.syncUIWithState(),await _()}catch(e){U("Failed to load data")}};window.handleCategoryFilter=async(e,t)=>{"all"===e&&e!==p.currentCategoryFilter&&(p.setCategoryFilter(e),E(),await _())};let $=()=>{g.init();let e={searchInput:document.getElementById("search-input"),minPriceInput:document.getElementById("min-price"),maxPriceInput:document.getElementById("max-price"),searchBtn:document.getElementById("search-btn"),sortSelect:document.getElementById("sort-select")};e.sortSelect&&(e.sortSelect.value=p.sortBy);let t=async()=>{let t=e.searchInput?.value.trim()||"",r=e.minPriceInput?.value?parseFloat(e.minPriceInput.value):null,a=e.maxPriceInput?.value?parseFloat(e.maxPriceInput.value):null,i=e.sortSelect?.value||"recommended";if(null!==r&&null!==a&&r>a){alert("Harga minimum tidak boleh lebih besar dari harga maksimum");return}p.setSearchQuery(t),p.setPriceRange(r,a),p.setSortBy(i);let n={search:document.getElementById("search-input-mobile"),minPrice:document.getElementById("min-price-mobile"),maxPrice:document.getElementById("max-price-mobile"),sort:document.getElementById("sort-select-mobile")};n.search&&(n.search.value=t),n.minPrice&&(n.minPrice.value=r||""),n.maxPrice&&(n.maxPrice.value=a||""),n.sort&&(n.sort.value=i),E(),g.updateBadge(),await _()};e.searchBtn&&e.searchBtn.addEventListener("click",t),[e.searchInput,e.minPriceInput,e.maxPriceInput].filter(Boolean).forEach(e=>{e.addEventListener("keypress",e=>{"Enter"===e.key&&(e.preventDefault(),t())})}),e.sortSelect&&e.sortSelect.addEventListener("change",async e=>{let t=e.target.value;p.setSortBy(t);let r=document.getElementById("sort-select-mobile");r&&(r.value=t),E(),g.updateBadge(),await _()}),[e.minPriceInput,e.maxPriceInput].filter(Boolean).forEach(e=>{e.addEventListener("input",e=>{let t=e.target.value;t&&0>parseFloat(t)&&(e.target.value="")})})},I=(e,t)=>{if(!e||0===e.length)return e;let r=[...e];switch(t){case"price_low":r.sort((e,t)=>(0,a.HY)(e)-(0,a.HY)(t));break;case"price_high":r.sort((e,t)=>{let r=(0,a.HY)(e);return(0,a.HY)(t)-r});break;default:r.sort((e,t)=>{if(e.is_featured&&!t.is_featured)return -1;if(!e.is_featured&&t.is_featured)return 1;let r=parseInt(e.id||e.product_id||0);return parseInt(t.id||t.product_id||0)-r})}return r},E=()=>{let e=document.getElementById("active-filters"),t=document.getElementById("active-filters-container");if(!e||!t)return;let r=[];if(p.searchQuery&&r.push({type:"search",label:`"${p.searchQuery}"`,value:p.searchQuery}),null!==p.priceRange.min||null!==p.priceRange.max){let e="";null!==p.priceRange.min&&null!==p.priceRange.max?e=`Rp ${(0,a.T4)(p.priceRange.min)} - Rp ${(0,a.T4)(p.priceRange.max)}`:null!==p.priceRange.min?e=`≥ Rp ${(0,a.T4)(p.priceRange.min)}`:null!==p.priceRange.max&&(e=`≤ Rp ${(0,a.T4)(p.priceRange.max)}`),r.push({type:"price",label:e,value:p.priceRange})}"recommended"!==p.sortBy&&r.push({type:"sort",label:`Urutan: ${{price_low:"Harga Terendah",price_high:"Harga Tertinggi"}[p.sortBy]}`,value:p.sortBy}),r.length>0?(e.classList.remove("hidden"),t.innerHTML=r.map(e=>`
    ...
    `).join("")):e.classList.add("hidden")};window.removeActiveFilter=async e=>{switch(e){case"search":p.setSearchQuery(""),[document.getElementById("search-input"),document.getElementById("search-input-mobile")].filter(Boolean).forEach(e=>e.value="");break;case"price":p.setPriceRange(null,null),[document.getElementById("min-price"),document.getElementById("max-price"),document.getElementById("min-price-mobile"),document.getElementById("max-price-mobile")].filter(Boolean).forEach(e=>e.value="");break;case"sort":p.setSortBy("recommended"),[document.getElementById("sort-select"),document.getElementById("sort-select-mobile")].filter(Boolean).forEach(e=>e.value="recommended")}E(),g.updateBadge(),await _()};let B=async()=>{try{let e;if("function"==typeof i.Ew.getCategories)e=await i.Ew.getCategories();else if("function"==typeof i.Ew.getCategoryList)e=await i.Ew.getCategoryList();else{p.categories=[];return}let t=(e&&Array.isArray(e.categories)?e.categories:e&&Array.isArray(e.data)?e.data:e&&Array.isArray(e)?e:[]).filter(e=>null===e.parent_id);p.categories=t.map(e=>{let t=e.name||e.category_name||"Unknown Category",r=e.slug||e.category_slug||t.toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"").replace(/-+/g,"-").replace(/^-|-$/g,"");return{id:e.id||e.category_id,name:t,slug:r,total_products:e.total_products||0}})}catch(e){p.categories=[]}},P=()=>{let e=document.getElementById(n.filtersId);if(!e)return;let t=e.querySelector(".category-filters-list");if(!t)return;let r=p.categories.filter(e=>e&&e.name&&e.name.trim().length>0),a=localStorage.getItem("category-display-style")||"pill";"dot"===a?t.classList.add("dot-separated"):t.classList.remove("dot-separated");let i=[];"dot"===a?(i.push(`
      <button
        class="category-filter-item ${"all"===p.currentCategoryFilter?"active":""}"
        data-category="all"
  ...
  `,a=`
    ...
    <div class="boxed-container">
      <div class="products-grid-5">
        ${p.currentProducts.map(e=>S(e)).join("")}
      </div>
    </div>
  `;e&&e.classList.add("hidden"),t&&(t.innerHTML=a,t.classList.remove("hidden")),L()},S=e=>{let t=e.primary_image_url||e.image_url||e.product_image,r=e.title||e.product_title||e.name||"Unknown Product",i=r.charAt(0).toUpperCase(),n=parseFloat(e.price_regular||e.product_price_regular||0),o=parseFloat(e.price_discount||e.product_price_discount||0),l=(0,a.T9)(e),s=e.id||e.product_id,d=e.slug||e.product_slug||(e.title||e.product_title||"").toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"").replace(/-+/g,"-").replace(/^-|-$/g,"")||s;return`
  ...
  `},L=()=>{document.querySelectorAll("[data-product-id]").forEach(e=>{e.addEventListener("click",t=>{t.preventDefault();let r=e.dataset.productSlug,a=e.dataset.productId,i=r&&"undefined"!==r&&r!==a?`/products/${r}`:`/products/${a}`;window.location.href=i}),e.addEventListener("keydown",t=>{if("Enter"===t.key||" "===t.key){t.preventDefault();let r=e.dataset.productSlug,a=e.dataset.productId,i=r&&"undefined"!==r&&r!==a?`/products/${r}`:`/products/${a}`;window.location.href=i}}),e.setAttribute("tabindex","0"),e.style.cursor="pointer",e.addEventListener("mouseenter",()=>{e.style.transform="translateY(-2px)"}),e.addEventListener("mouseleave",()=>{e.style.transform="translateY(0)"})})};window.handleProductClick=(e,t)=>{let r=t&&"undefined"!==t&&t!==e?`/products/${t}`:`/products/${e}`;window.location.href=r};let C=()=>{let e=document.getElementById("pagination-container");if(!e)return;let t=Math.ceil(p.totalItems/p.limit),r=p.page;if(t<=1){e.classList.add("hidden");return}e.classList.remove("hidden");let a=document.getElementById("pagination-content");if(!a)return;let i="";r>1&&(i+=`
      <button 
    ...
    `),a.innerHTML=i};window.goToPage=async e=>{p.setPage(e),await _(),d.scrollToProducts(!0)};let F=()=>{let e=document.getElementById(n.loadingId),t=document.getElementById(n.gridId);e&&e.classList.remove("hidden"),t&&t.classList.add("hidden")},T=()=>{let e=document.getElementById(n.loadingId),t=document.getElementById(n.gridId);if(e&&e.classList.add("hidden"),t){let e=p.searchQuery||"all"!==p.currentCategoryFilter||null!==p.priceRange.min||null!==p.priceRange.max||"recommended"!==p.sortBy;t.innerHTML=`
      <div class="boxed-container">
    ...
    `,t.classList.remove("hidden")}},U=e=>{let t=document.getElementById(n.loadingId),r=document.getElementById(n.gridId);t&&t.classList.add("hidden"),r&&(r.innerHTML=`
      <div class="boxed-container">
        <div class="text-center py-16">
    ...
    `,r.classList.remove("hidden"))};window.clearAllFilters=async()=>{p.reset(),["search-input","search-input-mobile","min-price","min-price-mobile","max-price","max-price-mobile"].forEach(e=>{let t=document.getElementById(e);t&&(t.value="")}),["sort-select","sort-select-mobile"].forEach(e=>{let t=document.getElementById(e);t&&(t.value="recommended")}),document.querySelectorAll(".category-filter-item").forEach(e=>{e.classList.remove("active"),"all"===e.dataset.category&&e.classList.add("active")}),E(),g.updateBadge(),await _()},window.reloadProducts=async()=>{p.reset(),["search-input","search-input-mobile","min-price","min-price-mobile","max-price","max-price-mobile"].forEach(e=>{let t=document.getElementById(e);t&&(t.value="")}),["sort-select","sort-select-mobile"].forEach(e=>{let t=document.getElementById(e);t&&(t.value="recommended")}),document.querySelectorAll(".category-filter-item").forEach(e=>{e.classList.remove("active"),"all"===e.dataset.category&&e.classList.add("active")}),E(),g.updateBadge(),d.scrollToTop(!0),await k()},window.debugProducts={testURL:()=>{let e=s.getCurrentParams();return s.updateURL({page:2,search:"test",category:"category-1",min_price:1e3,max_price:5e3,sort:"price_low"}),{currentParams:e,newUrl:window.location.href}},testAutoScroll:()=>(d.scrollToTop(!0),setTimeout(()=>{d.scrollToProducts(!0)},2e3),{scrollOffset:d.scrollOffset}),getState:()=>({urlParams:s.getCurrentParams(),stateParams:p.getFilterState(),categories:p.categories.length,currentProducts:p.currentProducts.length,totalItems:p.totalItems,isLoading:p.isLoading,mobileFilterOpen:g.isOpen}),reload:()=>{window.reloadProducts()}}}};